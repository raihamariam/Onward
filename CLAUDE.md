...
**Zero-spend runtime (decided in Phase 3).** Onward's *running system* must not bill against any paid API or against the Anthropic subscription this development session itself runs on — those are two separate things, and conflating them was the mistake to avoid. The model provider behind incident interpretation is swappable (`app/interpret/providers/`, `MODEL_PROVIDER` env var) specifically so the runtime can sit on a free tier while development tooling (this Claude Code session) remains unaffected. Default is now **Groq** (`qwen/qwen3.8-27b`) — Gemini was the original default but its free-tier daily quota proved too low for actual development/live-gate iteration (~20 requests/day on this project's key; see `gemini.py`'s docstring for the live-verified number). This is exactly why the provider abstraction exists: swapping the active provider was a one-line default change (`get_provider()`) plus one new provider class, zero changes to the schema, retry logic, review policy, or either Gemini or Anthropic's own implementations — both remain fully working, just not active. A free tier's terms typically permit the provider to use submitted content to improve their products — so only synthetic/non-sensitive hackathon data (the seeded assets, test incident descriptions) may ever be sent to it; this is a hard rule, not a preference, until a paid tier or a provider with different terms is deliberately chosen.
