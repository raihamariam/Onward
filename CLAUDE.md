...
1. **The model call moves into the decision-engine service, not into an n8n node.** The vault had n8n calling Claude directly inside WF‑02. Instead, the FastAPI service owns the prompt, the forced structured-output schema, and the validate‑or‑retry logic, as versioned, unit-testable Python — and, as of Phase 3's provider cleanup, this is also exactly why swapping the active provider (Gemini → Groq) never touched n8n at all. n8n still triggers it and still owns *when* it happens — orchestration stays in n8n — but the fragile, hard-to-diff, hard-to-unit-test part (prompt + schema enforcement) lives in code that can be tested with mocked API responses. n8n node configuration cannot be unit tested; a Python function can.
2. **There is no agent in this system.** The vault already resisted multi-agent hype, but still called its AI step an "agent." It isn't one — it never loops, never chooses its own tools, never takes multiple turns. It is a single-shot, schema-forced LLM function call: `interpret_incident(inputs) -> IncidentIntelligence`. Calling it a function instead of an agent is not a rename for its own sake — it changes how you build and test it (fixture-in, schema-out, no tool-loop failure modes to defend against).
...
|---|---|
| **LangChain / LangGraph** for the incident-interpretation step | It's a single forced-schema LLM call, not a chain or a graph. LangChain adds an abstraction layer over the underlying provider SDK that makes the call harder to unit test and debug, for zero functional gain at this scope — true regardless of which provider (Groq, Gemini, or anything else) sits behind it. |
...
| **`instructor` library** for structured model output | Genuinely useful for exactly this problem (validated-Pydantic-object-from-LLM-call with retry) — but it wraps a specific provider SDK client directly, and the model call already lives in one place (decision-engine's `/incident/interpret`, behind the provider abstraction) where we control the retry loop explicitly in ~20 lines. Adding a dependency to save 20 lines of code we need to fully understand anyway (for the safety/confidence branching) isn't worth it, and it would fight the multi-provider design (it's built around one SDK, not `InterpretationProvider`). Reconsider if the retry/validation logic grows materially more complex. |
...
**Zero-spend runtime (decided in Phase 3).** Onward's *running system* must not bill against any paid API or against the Anthropic subscription this development session itself runs on (that subscription is this Claude Code session's own tooling — unrelated to Onward's runtime, and conflating the two was the original mistake to avoid). The model provider behind incident interpretation is swappable (`app/interpret/providers/`, `MODEL_PROVIDER` env var) specifically so the runtime can sit on a free tier while development tooling remains unaffected. Active provider: **Groq** (`qwen/qwen3.8-27b`). Gemini was the original default but its free-tier daily quota proved too low for actual development/live-gate iteration (~20 requests/day on this project's key; see `gemini.py`'s docstring for the live-verified number) — it's kept as a small, working manual fallback (`MODEL_PROVIDER=gemini`), not deleted. Anthropic was removed from the runtime entirely (Phase 3 cleanup): it was never active, only ever a second unused swap-target duplicating Gemini's job as a fallback with no distinct reason to keep both. This is exactly why the provider abstraction exists: swapping the active provider was a one-line default change (`get_provider()`) plus one new provider class, zero changes to the schema, retry logic, or review policy. A free tier's terms typically permit the provider to use submitted content to improve their products — so only synthetic/non-sensitive hackathon data (the seeded assets, test incident descriptions) may ever be sent to it; this is a hard rule, not a preference, until a paid tier or a provider with different terms is deliberately chosen.
...
| **AI/model layer (Groq / qwen-3.8-27b by default, inside decision-engine)** | Interpreting text+image+trusted-asset-context into a structured `IncidentIntelligence` (`app/schemas/incident.py`) via one forced-schema call (Groq's strict `json_schema` response format) — symptoms, category, severity, safety risk, confidence, review flag; explaining a chosen plan in plain language for the UI/demo (later). Provider is swappable (`app/interpret/providers/`, `MODEL_PROVIDER` env var — `groq` default, `gemini` available as fallback); §5's zero-spend-runtime principle governs the default, not a hard dependency on any one vendor | Deciding room capacity, technician availability, inventory counts, event timing, whether a plan is valid, or anything requiring "the current state of the world" — enforced structurally: no field in the schema can hold an operational fact (see `test_never_invents_operational_facts`) |
...
The AI component is one **single-shot, schema-forced interpretation function**: `interpret_incident(photo, text, asset_context, retrieved_chunks) -> IncidentIntelligence`. It runs once per incident, does not choose its own tools, does not take multiple turns, and cannot invoke anything that changes state.
...
| `incidents` | Every reported issue: raw input, structured `IncidentIntelligence`, status |
...
│       │   │   └── providers/    One class per model provider (groq_provider.py active, gemini.py fallback)
